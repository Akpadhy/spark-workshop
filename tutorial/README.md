...
Before we tell you how to build and run the examples, note that there are many warnings and even a few error messages you'll see in the output. Most are harmless, like the following examples:

```
01/02/14 13:22:33 ERROR AbstractNioSelector: Interrupted while wait for resources to be released #2
...
01/02/14 13:23:37 WARN LoadSnappy: Snappy native library is available
01/02/14 13:23:37 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
01/02/14 13:23:37 WARN LoadSnappy: Snappy native library not loaded
```

The first `ERROR` is due to the constrained way we are running Spark. The warnings about "native" libraries are common when running with any Hadoop I/O libraries in a cluster that isn't configured to use the native C libraries for components like compression libraries.

When you run the Hadoop examples, you'll see even more logging output.

The only real error you can't ignore is an out of memory error, which might occur when you run all the examples in one session. By default, *Activator* is configured to use 4GB by default and we recommend that you allocate 10GB to your virtual machine in VMWare or VirtualBox. If you can't allocate that much memory or you have allocated even more memory, then start with the `--mem N` flag where N is the number of MB to use. For example, to use 6GB:

```
spark-workshop/start.sh --mem 6144
```

or

```
spark-workshop/start.sh --mem 6144 shell
```

In any case, if you run out of memory, exit Activator, restart it with the `start.sh` script, and continue where you left off.
